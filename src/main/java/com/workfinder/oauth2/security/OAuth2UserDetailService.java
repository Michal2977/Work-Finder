...
import com.workfinder.oauth2.OAuth2UserDetails;
...
public class OAuth2UserDetailService extends DefaultOAuth2UserService {
    ...
    public OAuth2User processOAuth2User(String registrationId, Map<String,Object> attributes){
        ...
        if (user == null){
            ...
            user = updateExistingUser(user,provider,custom);
        }
        SecurityUserDetails userDetails = new SecurityUserDetails(user,attributes);
        userDetails.setLinked(linked);
    ...
    private User updateExistingUser(User user,OAuth2UserProvider provider,OAuth2UserCustom custom){
